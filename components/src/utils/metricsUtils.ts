...
const calculateLeftRightAngleDifference = (frame: PoseFrame, left: string, right: string): number => {
  const leftPoint = frame.find(kp => kp.name === left);
  const rightPoint = frame.find(kp => kp.name === right);
  if (!leftPoint || !rightPoint) return 0;
  return Math.abs(leftPoint.y - rightPoint.y);
};

const calculateTrunkLean = (frame: PoseFrame): number => {
  const shoulder = frame.find(kp => kp.name === 'left_shoulder') || frame.find(kp => kp.name === 'right_shoulder');
  const hip = frame.find(kp => kp.name === 'left_hip') || frame.find(kp => kp.name === 'right_hip');
...
const calculateArmsDrop = (frame: PoseFrame): number => {
  const shoulder = frame.find(kp => kp.name === 'left_shoulder') || frame.find(kp => kp.name === 'right_shoulder');
  const wrist = frame.find(kp => kp.name === 'left_wrist') || frame.find(kp => kp.name === 'right_wrist');
...
const detectKneeValgus = (frame: PoseFrame, threshold: number): boolean => {
  const knee = frame.find(kp => kp.name === 'left_knee') || frame.find(kp => kp.name === 'right_knee');
  const ankle = frame.find(kp => kp.name === 'left_ankle') || frame.find(kp => kp.name === 'right_ankle');
  const hip = frame.find(kp => kp.name === 'left_hip') || frame.find(kp => kp.name === 'right_hip');
